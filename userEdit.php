		   ...
		   <?php
            ...
            if($row = mysqli_fetch_array($results)){
			  ...
			  <form action="" method="post" style="margin-left: 20px;"> 
				 <input type="text" name="fname" value="<?php echo $row['names'];?>" placeholder="Enter your first name" size="40"/> <br/><br/>

				 <input type="text" name="email" value="<?php echo $row['email'];?>"placeholder="Enter your email" size="40"/> <br/>
				 <br/>
				 <select name = "gender">
		...
		<?php
          ...
          if(isset($_POST['up'])){	
            ...
            echo $role = $_POST['role'];			
			 echo "<br/>";
		    $q = "UPDATE user SET  names ='$fname', email='$email', gender='$gender', role='$role' WHERE id=".$id; 
			//die($q);
			$results = mysqli_query($con,$q);   
			if (!$results){
			   die("Something went wrong.".mysqli_error($con));
